...
export default function ClientModal({ client, providerId, onClose, onSaved }) {
  const [form, setForm] = useState(
    client
      ? { ...client, rate: client.rate?.toString() ?? '' }
      : { ...BLANK }
  )
  ...
  async function handleSubmit(e) {
    ...
    if (client) {
      ...
      ;({ error: err } = await supabase.from('clients').insert({ ...payload, provider_id: providerId }))
    }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-end md:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white rounded-t-2xl">
          <h2 className="font-bold text-lg text-slate-900">{client ? 'Edit Client' : 'Add Client'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Billing Type</label>
              <select
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium disabled:opacity-50 transition-colors">
